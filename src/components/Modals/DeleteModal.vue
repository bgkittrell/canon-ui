...
const { message, title, onConfirm } = defineProps<{
  message: string
  title: string
  onConfirm: () => void
}>()
...
<template>
  <Teleport to="#modals">
    ...
    Leaving: "ease-in duration-200"
      ...
      To: "opacity-0"
  -->
      <div class="fixed inset-0 bg-slate-800 bg-opacity-60 transition-opacity"></div>
          ...
          >
            <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
              <div class="sm:flex sm:items-start">
                <div
                  class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-danger sm:mx-0 sm:h-10 sm:w-10"
                >
                  <svg
            ...
            <div class="bg-slate-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
              <button
                type="button"
                class="inline-flex w-full justify-center rounded-md bg-danger px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-danger sm:ml-3 sm:w-auto"
                @click="handleDelete"
              >
                Delete
              </button>
