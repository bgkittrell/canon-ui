...
<template>
  <li>
    <router-link
      :to="item.route"
      class="group relative flex items-center gap-2.5 rounded-sm overflow-hidden py-2 px-4 font-medium text-slate-600 duration-300 ease-in-out hover:bg-primary hover:bg-opacity-10 dark:text-slate-50"
      @click.prevent="handleItemClick"
      :class="{
        'bg-primary bg-opacity-10': sidebarStore.page === item.label
      }"
    >
      <span><component :is="item.icon" class="w-4 h-4"></component></span>
